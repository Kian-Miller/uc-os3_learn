...
##### 操作：
1. W25Q16是通过SPI协议与MCU进行数据传输
   1. SPI GPIO的初始化，引脚复用和SPI1初始化设置，配置SPI1 SCK, MISO和MOSI引脚(PB3, PB4, PB5) F_CS引脚(PB0)
   2. 操作SPI发送和接收缓存区，读取数据
2. 按页读取数据，要注意在操作数据时跨页跨扇要单独处理
##### 现象：
1. 数据写入到空闲FLASH
2. 断电重启后可以再次读取数据并显示
...
#### 实验五
##### 目的：
1. 使用外部FLASH W25Q16，持久化保存数据，并在重新上电运行时读取数据
##### 操作：
1. 按下K#,向空闲FLASH内写入数据(注意数据要对齐,写入读取时是按照32位操作的)
2. 断电，重新上电后，按下K*，从空闲内存块读取数据，并显示到OLED上
##### 现象：
1. 能够持续的往W25Q16写入计数值，并能够读取写入的数据